...
resource_key = 'payroll.module'
action_key = 'list'


@register(resource_key, action_key)
class PayrollsListScope(ScopeStrategy):
    """
    ...
    def resolve(self, user) -> ScopeResult:
        ...
        try:
            res_id = Resource.objects.only("id").get(key=resource_key).id
            act_id = Action.objects.only("id").get(key=action_key).id
    ...
    def filter_queryset(self, qs: QuerySet, user) -> QuerySet:
        ...
        if res.dept_ids:
            flt |= Q(department_id__in=list(res.dept_ids)) | Q(department_id__in=list(res.dept_ids))
        if res.self_subdept_only and getattr(user, "top_level_department_id", None) is not None:
            flt |= Q(department_id=user.top_level_department_id)
